let maskingFunc = {
	...
	/*
	 * ※ 이메일 마스킹 ex1) 원본 데이터 : abcdefg12345@naver.com 변경 데이터 :
	 * ab**********@naver.com ex2) 원본 데이터 : abcdefg12345@naver.com 변경 데이터 :
	 * ab**********@nav******
	 */
	email : function(str) {
		let originStr = str;
		...
		if (this.checkNull(originStr) == true
			...
			// return originStr.toString().replace(new RegExp('.(?=.{0,' +
			// strLength + '}@)', 'g'), '*');
	...
	/*
	 * ※ 휴대폰 번호 마스킹 ex1) 원본 데이터 : 01012345678, 변경 데이터 : 010****5678 ex2) 원본 데이터 :
	 * 010-1234-5678, 변경 데이터 : 010-****-5678 ex3) 원본 데이터 : 0111234567, 변경 데이터 :
	 * 011***4567 ex4) 원본 데이터 : 011-123-4567, 변경 데이터 : 011-***-4567
	 */
	phone : function(str) {
		let originStr = str;
		let phoneStr;
		...
		if (originStr.toString().split('-').length != 3) { // 1) -가 없는 경우
			phoneStr = originStr.length < 11 ? originStr.match(/\d{10}/gi)
			...
			if (originStr.length < 11) { // 1.1) 0110000000
				maskingStr = originStr.toString().replace(
						phoneStr,
						phoneStr.toString().replace(/(\d{3})(\d{3})(\d{4})/gi,
								'$1***$3'));
			} else { // 1.2) 01000000000
				maskingStr = originStr.toString().replace(
						phoneStr,
						phoneStr.toString().replace(/(\d{3})(\d{4})(\d{4})/gi,
								'$1****$3'));
			}
		} else { // 2) -가 있는 경우
			...
			if (/-[0-9]{3}-/.test(phoneStr)) { // 2.1) 00-000-0000
				maskingStr = originStr.toString().replace(phoneStr,
	...
	/*
	 * ※ 주민등록 번호 마스킹 (Resident Registration Number, RRN Masking) ex1) 원본 데이터 :
	 * 990101-1234567, 변경 데이터 : 990101-1****** ex2) 원본 데이터 : 9901011234567, 변경
	 * 데이터 : 9901011******
	 */
	rrn : function(str) {
		let originStr = str;
	...
	/*
	 * ※ 이름 마스킹 ex1) 원본 데이터 : 갓댐희, 변경 데이터 : 갓댐* ex2) 원본 데이터 : 하늘에수, 변경 데이터 :
	 * 하늘** ex3) 원본 데이터 : 갓댐, 변경 데이터 : 갓*
	 */
	name : function(str) {
		let originStr = str;
		let maskingStr;
